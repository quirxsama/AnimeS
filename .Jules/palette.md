...
## 2025-02-26 - Framer Motion vs Keyboard Focus
**Learning:** Framer Motion's `whileHover` prop does not automatically trigger for keyboard focus states, leaving keyboard users without visual feedback for interactive elements.
**Action:** Replace `whileHover` with CSS `group-hover` and `group-focus-visible` classes (or use `whileFocus`) to ensure consistent states for both mouse and keyboard users.
